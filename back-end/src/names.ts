...
export const createNames = async (game: any) => {
...
const loadNames = async (req: any, res: any, next: any) => {
...
const checkCompletion = async (game: any, namesType: any) => {
  if (game.phase !== 'play') return [];

  const userToName = (user: any) => ({ user: user, text: '' });
...
router.get('/', joinPhase, async (req: any, res) => {
  try {
    ...
    if (req.game.phase === 'play') {
      const userElem = namesType.names.find((elem: any) =>
      ...
      return res.send({
        phase: 'read',
        names: namesType.names.map((elem: any) => elem.text),
      });
    } else {
      return res.send({
        phase: 'end',
...
router.put('/', async (req: any, res) => {
  ...
  let userIndex = namesType.names.findIndex((elem: any) =>
    elem.user._id.equals(req.user._id)
  );
  if (userIndex === -1) {
    statusCode = 201;
